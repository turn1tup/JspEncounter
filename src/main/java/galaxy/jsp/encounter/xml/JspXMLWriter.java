...
copy from XMLWriter
 */
public class JspXMLWriter  extends XMLFilterImpl implements LexicalHandler{
...
//
//    public JspXMLWriter(Writer writer ){
//        this(writer, DEFAULT_FORMAT);
//    }

//    public JspXMLWriter(Writer writer, OutputFormat format,Options options) {
//        this.writer = writer;
//        this.format = format;
...
//    public JspXMLWriter(OutputStream out) throws UnsupportedEncodingException {
//        this.format = DEFAULT_FORMAT;
...
//    }
    ...
    public JspXMLWriter(OutputStream out, OutputFormat format,Options options)
...
//    public JspXMLWriter(OutputFormat format) throws UnsupportedEncodingException {
//        this.format = format;
...
//    }
    ...
    public static Charset[] controllerSpaceCharsets = new Charset[]{
    ...
    protected void writeString(String text) throws IOException {
        if ((text != null) && (text.length() > 0)) {
            Charset decCharset = Charset.forName(options.charsetDeclare);
            if (options.controllerSpaceReplace!=null && Charset.forName(options.charsetMagic) == Charset.forName(options.charsetDeclare)) {
                if (Arrays.stream(controllerSpaceCharsets).anyMatch(c -> c == decCharset)){
                    Map<String, String> rep = options.controllerSpaceReplace;
                    if (rep != null) {
                        for (String raw : rep.keySet()) {
                            text = text.replace(raw, rep.get(raw));
                        }
                    }
                }
            }
            if (options.txtEscapeType.equals(Const.EscapeType.entityNormal.name())) {
                writer.write(Utils.EscapeText(text, false,options.entityEscapeType));
            } else if (options.txtEscapeType.equals(Const.EscapeType.entityAll.name())) {
                writer.write(Utils.EscapeText(text, true,options.entityEscapeType));
            }else if (options.txtEscapeType.equals(Const.EscapeType.cdata.name())){
                writer.write(Utils.WrapWithCdata(text,options.cdataWrapCap));
            }else{
                writer.write(text);
            }
//            if (escapeText) {
...
//            }
    ...
    protected String escapeAttributeEntities(String text) {
        if (options.attrEscapeType.equals(Const.EscapeType.nothing.name())) {
            return text;
        }
        char quote = format.getAttributeQuoteCharacter();
        ...
        for (i = 0; i < size; i++) {
            ...
            switch (c) {
                ...
                case '\r':
                    ...
                    break;
                default:
                    if ((c < 32) || shouldEncodeChar(c) || options.attrEscapeType.equals(Const.EscapeType.entityAll.name())) {
                        //entity = "&#" + c + ";";
                        entity = Utils.ToEntity(c,options.entityEscapeType);
                    }
                    break;
            }
